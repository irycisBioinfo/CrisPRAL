...
if ($single_end eq "FALSE"){#Paired-End Section:
	...
	if ($Adapter_R1 eq 'Empty') {
		...
		system("$PATH/bin/cutadapt/cutadapt --revcomp -j 0 -a $Adapter_R1 -n 5 -o $tmpdir/R1_filteredA.fastq $r1");
		system("$PATH/bin/cutadapt/cutadapt --revcomp -j 0 -a $Adapter_R2 -n 5 -o $tmpdir/R2_filteredA.fastq $r2");
	...
	if ($F_Primer1 eq 'Empty') {
		...
		system("$PATH/bin/cutadapt/cutadapt --revcomp -j 0 -g $F_Primer1 -a $F_Primer2 -n 5 -o $tmpdir/R1_filteredP.fastq $tmpdir/goodA_R1.fastq");
		system("$PATH/bin/cutadapt/cutadapt --revcomp -j 0 -g $R_Primer1 -a $R_Primer2 -n 5 -o $tmpdir/R2_filteredP.fastq $tmpdir/goodA_R2.fastq");
...
if ($F_Primer1 eq 'Empty') {
	...
	system("$PATH/bin/cutadapt/cutadapt --revcomp -j 0 -m 10 -g $F_Primer1 -a $F_Primer2 -n 5 -o $tmpdir/R1_filteredP.fastq $tmpdir/goodA_R1.fastq");
	system("$PATH/bin/prinseq/prinseq-lite.pl -fastq $tmpdir/R1_filteredP.fastq -trim_left $trimP1 -trim_right $trimP2 -out_good $tmpdir/good_R1");
	print("\nDone\n");
