...
server <- function(input, output) {
  ...
  output$print = renderPrint(Target_FileInput())
  ...
  observeEvent(input$Accept, {
    ...
    withProgress(message = "Performing analisis", {
      ...
      if(!is.null(datos$Target)){datos$Target = readDNAStringSet(input$Target$datapath)}
  ...
  observeEvent(c(input$tabla_rows_selected, input$alnType, input$alnTo), {
    withProgress(message = "Performing analisis", {
      ...
      if(input$alnTo == "Reference")
      {
        ...
        req(input$Target)
        aln = pairwiseAlignment(Target_FileInput(), Seq, type = input$alnType)
      }
