...
use Cwd 'abs_path';
$PATH = abs_path($0);
$PATH =~ s/\/pipeline_v3.pl//;

@Options = (
		
		{OPT=>"r1=s",	VAR=>\$r1,	DESC=>"Reads 1"},
		{OPT=>"r2=s",	VAR=>\$r2,	DESC=>"Reads 2"},
		{OPT=>"trimP1=s", VAR=>\$trimP1, DEFAULT => "0",DESC=>"1st Primer trimming options"},
		{OPT=>"trimP2=s", VAR=>\$trimP2, DEFAULT => "0",DESC=>"2nd Primer trimming options"},
...
#system('cutadapt -f fastq -a ADAPTER -o R1_noAdapt $r1')
#system('cutadapt -f fastq -g ADAPTER -o R2_noAdapt $r2')
