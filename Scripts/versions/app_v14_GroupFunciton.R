        ...
        tabsetPanel(id = 'Visualization', selected = 'Clusterization_&_Alignment',
          tabPanel("Clusterization & Alignment", value = "Clusterization_&_Alignment",
            ...
            checkboxInput("display_unaligned", 
                          p(strong("Display unaligned fasta")), value = FALSE),
            conditionalPanel(
...
server <- function(input, output, session) {
  ...
  output$checkFP1 <- renderText(paste("Read 1, 5' end: ",PrimerR1_5(), sep = ''))
  output$checkFP2 <- renderText(paste("Read 1, 3' end: ",PrimerR1_3(), sep = ''))
  output$checkRP1 <- renderText(paste("Read 2, 5' end: ",PrimerR2_5(), sep = ''))
  output$checkRP2 <- renderText(paste("Read 2, 3' end: ",PrimerR2_3(), sep = ''))
  ...
  observeEvent(input$Accept, {
    ...
    withProgress(message = "Performing analisis: ", detail = 'Clustering, may take a while...', {
      ...
      incProgress( 1/6 ,detail = 'Alignments... renaming')
      
      datos$aln = ls_ClusterAln_Ref$aln
      
      incProgress( 1/6 ,detail = 'Alignments')
      ...
      #PDF formatting
      output$Print <- renderText(datos$Tabla[sort.default(rows_selected()),])
      output$Print2 <- renderText(paste("Total amount of Reads: ", 
                                         Total_Abundance, sep = ''))
      
      #Data with named groups for FASTAS downloads.
      datos$namedSequences <- datos$Tabla %>% left_join(datos$fasta) %>% select(ID, Sequence) %>%
      ...
      if(!is.null(Target_FileInput())){ 
        ...
        output$Target_Location <- renderText(paste('Target is found in Cluster:', Target_loc, 
                                                    sep = ' '))
      ...
      incProgress( 1/6 ,message = 'Plotting...', detail = 'Pie chart') #User feedback
      ...
      incProgress( 1/7 ,detail = 'Mutation Frequency') #User feedback
      ...
      #- Dumped count
       ...
       output$Dump = renderText(paste('Filtered Clusters because of size restrictions: ',
 ...
 tabla_rows_selected <- eventReactive(c(input$tablaR_rows_selected, 
    ...
    withProgress(message = "Performing analisis", {
      ...
      if (!is.null(tabla_rows_selected()))
      {
        datos$text = as.data.frame(c(
          paste(pos, sep = '', collapse = ''), #Not working well enough, fails sometimes
  ...
  input_filename_group <- reactive({paste( input_filename_generic(), 
                                     '_Group', tabla_rows_selected(), sep='')})
  ...
  callModule(downloadFASTA, 'downloadFASTAS', #This isnt working as rows_selected() is a reactive variable
