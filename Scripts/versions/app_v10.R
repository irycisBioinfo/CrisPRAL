...
source(paste(CompletePATH, '/Modules/position_vector_module.R', sep = ''))
...
ui <- fluidPage(# Application title
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      wellPanel( fileInput("Target", "Target (optional)", accept = c('.fasta','.fastq')) ),
...
server <- function(input, output) {
  ...
  Target = reactive({
    #'Target' is an optional field
    req(input$Target)
    readDNAStringSet(input$Target$datapath)
  })
  
  observeEvent(input$Accept, {
    
    req(input$R1, input$R2, input$Reference) #Prevents App from crashing by only executing if R1 R2 and Reference exist
  ...
  observeEvent(c(input$tabla_rows_selected, input$alnType, input$alnTo), {
    withProgress(message = "Performing analisis", {
      ...
      if(input$alnTo == "Reference")
      {
        ...
        aln = pairwiseAlignment(Target(), Seq, type = input$alnType)
      }
      ...
      pos = Position_vector(as.character(aln@pattern))
  ...
  output$downloadReport <- downloadHandler(
    ...
    content = function(file) {
      ...
      out <- render('reports.Rmd', pdf_document())
