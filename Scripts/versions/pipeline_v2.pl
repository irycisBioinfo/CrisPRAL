...
@Options = (
		...
		{OPT=>"trim=s", VAR=>\$trim, DEFAULT => "10",DESC=>"Primer trimming options"},
...
system("$PATH/bin/prinseq/prinseq-lite.pl -fastq $r1 -fastq2 $r2 -min_len $minLen -trim_left $trim -trim_right $trim -out_good good -out_bad bad");
