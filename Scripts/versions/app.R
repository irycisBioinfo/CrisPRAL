...
# 
# *Change Primer/Adapter filtering pipeline. Introduce Primer sequence directly/generate a fasta file to then read with -
# -DNASTRINGSET and feed to pipeline - provide wide range of options to user.
# #in pipelin_v4 add conditional loops for the pipeline method (need Val)
    ...
    sidebarPanel(
      ...
      wellPanel(
        ...
        numericInput(
          ...
          step = 10),
        wellPanel(
          #Turn this into a navBar() and provide default option to NULL
          textInput('Adapter','Residual adapter trimming'),
          fileInput('Adapter', 'From file (Fasta format)', accept = '.fasta'),
          fluidRow(
            column(4,
                   actionButton('resetA', 'Reset Input')
            ))
        ),
        #Introduce tab panel options
        navbarPage(title = p(strong('Primer trimming  (forward read):')),
          ...
          tabPanel(p(strong(("by sequence"))),
                   tabsetPanel(
                     tabPanel("Direct input",
                        hr(),
                        #Missing primer sequence for the reverse
                        textInput('F_Primer1',"5' sequence"),
                        textInput('F_Primer2',"3' sequence")),
                     tabPanel('File upload',
                        ...
                        p(em('The file should contain four lines, headers on line 1 and 3 starting with an arrow ">", 
                             first sequence being primer 1 for the Forward strand and second sequence primer 2. Reverse reads are
                             obtained automatically.')),
                        br(),
                        fluidRow(column(4, actionButton('resetP', 'Reset Input')),
                        br(),
                        br(),
                        br(),
...
server <- function(input, output) {
  ...
  PrimerR1_sequences <- reactive({
    validate(need(!is.null(Primer_FileInput()),'Valid primers have not been uploaded'))
    return(readDNAStringSet(Primer_FileInput()$datapath))
                        })
  PrimerR2_sequences <- reactive({
    validate(need(!is.null(Primer_FileInput()),'Valid primers have not been uploaded'))
    return(reverseComplement(PrimerR1_sequences()))
    })
  
  Primer_names <- reactive({names(PrimerR1_sequences())}) #This allows primer name customization by user
  
  PrimerR1_5 <- reactive({
    #No need for error check since PrimerR1/R2 already has them.
    return(as.character(PrimerR1_sequences()[Primer_names()[1]]))
  })
  PrimerR1_3 <- reactive({
    #No need for error check since PrimerR1/R2 already has them.
    return(as.character(PrimerR1_sequences()[Primer_names()[2]]))
  })
  PrimerR2_5 <- reactive({
    #No need for error check since PrimerR1/R2 already has them.
    return(as.character(PrimerR2_sequences()[Primer_names()[2]]))
  })
  PrimerR2_3 <- reactive({
    #No need for error check since PrimerR1/R2 already has them.
    return(as.character(PrimerR2_sequences()[Primer_names()[1]]))
  })
