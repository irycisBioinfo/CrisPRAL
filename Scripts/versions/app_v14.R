...
# system('export OPENSSL_CONF=/etc/ssl/openssl.cnf')

if (require('bookdown') == FALSE){
      ...
      mainPanel(
        ...
        tabsetPanel(id = 'Visualization', selected = 'Clusterization_&_Alignment',
          tabPanel("Clusterization & Alignment", value = "Clusterization_&_Alignment",
            ...
            uiOutput('downloadPAIR_FASTA_file')
          ),
                    ...
                    wellPanel(
                      ...
                      tabPanel('Download Reports',
                               ...
                               fluidRow(
                                 ...
                                 downloadButtonModule("downloadFASTAS", "Download FASTAS")))
                   )))
                  )
  )))
...
server <- function(input, output) {
  ...
  #This following set of observeEvents allows the user to change his mind on uploaded Adapter files.
  ...
  observeEvent(input$Accept, {
    ...
    withProgress(message = "Performing analisis: ", detail = 'Clustering, may take a while...', {
      ...
      setProgress(detail = 'Alignments')
      
      datos$fasta = read_tsv("cluster.tsv", col_names = FALSE)
      datos$cluster = read_tsv("cluster.bak.tsv", col_names = FALSE)
      
      if (isEmpty(colnames(datos$fasta))){#-If dataset is completely filtered due to any 
        #restriction app crashes
      ...
      ls_ClusterAln_Ref = Clusters_Alignments(datos$Sequences, datos$Ref) #Function in Functios.R 
      #module
      
      datos$aln = ls_ClusterAln_Ref$aln
      
      system("rm R1_* R2_* join* cluster* good* bad* final*"); #To prevent constant cashing in on 
      #the same data #26/11 removed .cluster removal for debugging
      
      datos$Tabla = inner_join(datos$Tabla_raw, ls_ClusterAln_Ref$tmp) %>% mutate(
        score = round(score,1), Freq = signif(Freq,2)) %>% 
        arrange(desc(Abundance))
      
      output$tablaR <- renderDT(datos$Tabla, selection = "single")
      output$tablaD <- renderDT(datos$Tabla) #Table with multiple selection for PDF formatting
      ...
      if(!is.null(Target_FileInput())){ 
        ...
        output$Target_Location <- renderPrint(paste('Target is found in Cluster:', Target_loc, 
                                                    sep = ' '))
        output$tablaT <- renderDT(datos$Tabla_Target, selection = "single")
        
      }else{
        output$Target_Location <- renderPrint(paste('No Target was introduced', 
                                                    sep = ' '))
      }
      
      setProgress(message = 'Plotting...', detail = 'Pie chart') #User feedback
      ...
      Groups <- Pie_chart_Groups(datos$Pie_data) #- Function in Functions.R module
      datos$Pie_data <- add_column(datos$Pie_data, Groups)
        
      PC_online <- plot_ly(datos$Pie_data, labels = ~Groups, values = ~Freq, type = 'pie',
                    textposition = NULL,
                    hoverinfo = 'text',
      ...
      setProgress(detail = 'Mutation Frequency') #User feedback
      ...
      LetterPerPosition_Total = LetterPerPosition %>% group_by(Position, Chr, Abundance) %>% 
        count(Chr) %>% filter(Chr != '') %>% transmute(Total = Abundance*n) %>% ungroup() %>%
      ...
      Insert_data <- Insert_data %>% mutate(Count = DFGraph_Data['Abundance'][group,]) %>% 
      ...
      MF <- plot_ly(LetterPerPosition_Norm, x = ~Position, y = ~Total, type = 'bar', 
                    name = ~Chr, color= ~Chr , colors = colorscheme) %>%
      ...
      DL <- plot_ly(Deletions_locations, x = ~Position, y = ~Total, type = 'bar',
                    name = 'Deletions') %>%
      ...
      #- Dumped count
       ...
       output$Dump = renderPrint(paste('Filtered Clusters because of size restrictions: ',
  ...
  #tabla_rows_selected allow for alignment selection regardless of using table calculated
  # with Reference or Target
 tabla_rows_selected <- eventReactive(c(input$tablaR_rows_selected, input$tablaT_rows_selected),{

    if(input$Alignment == 'Reference'){ return( input$tablaR_rows_selected )
    }else{
      return( input$tablaT_rows_selected )
    }})
  
  observeEvent(c(input$tablaR_rows_selected, input$tablaT_rows_selected, input$alnType, input$alnTo), {
    withProgress(message = "Performing analisis", {
      
      req(datos$Tabla)
      req(tabla_rows_selected()) #To prevent crashing WITHOUT any error being handed to you.
      
      S = datos$Tabla %>% filter(ID == datos$Tabla$ID[tabla_rows_selected()]) %>% 
        left_join(datos$fasta)
      Seq = DNAStringSet(S$Sequence)
      
      if(input$alnTo == "Reference")
      {
        datos$aln2 = pairwiseAlignment(datos$Ref, Seq, type = input$alnType)
      }else{
        datos$aln2 = pairwiseAlignment(datos$Target, Seq, type = input$alnType)
      }
      
      
      comp = character(length(datos$aln2@pattern))
      ref = unlist(strsplit(as.character(datos$aln2@pattern), split = ""))
      query = unlist(strsplit(as.character(datos$aln2@subject), split = ""))
      ...
      pos <- Position_vector(as.character(datos$aln2@pattern), ref) #Generates suitable positioning vector
      ...
      if (!is.null(tabla_rows_selected()))
      {
        ...
        if(input$alnTo == "Reference")
        {
          rownames(datos$text) = c("Position", "Reference", 
                                   datos$Tabla$ID[tabla_rows_selected()], "Comparisson")
        }else{
      ...
      output$Blast_Search <- renderUI({column(3, wellPanel(helpText(
        a(div(img(src="BLASTn.png")), 
      ...
      output$downloadPAIR_FASTA_file <- renderUI({
        br()
        downloadButtonModule('downloadPAIR1', 'Download Pairwise alignment')
        hr()
        br()
        downloadButtonModule('downloadFASTA1', 'Download FASTA')
      })
    })
  })
  
  input_filename_generic <- reactive({paste(str_replace(#Removes posible double underscore "_"
                              str_replace(#Removes Read number ID
                                substring(text = input$R1$name, 
                                          first = 1, 
                                          last = nchar(input$R1$name)-6)
                                ,'R1',''), '__',''), sep='')})
  input_filename_results <- reactive({paste( input_filename_generic(), '_Results', sep='' )})
  input_filename_group <- reactive({paste( input_filename_generic(), '_Group', tabla_rows_selected(), sep='')})
  input_filename_pair <- reactive({paste( input_filename_group() , '_pairwise_alignment', sep='')})
  ...
  output$downloadReport <- downloadHandler(
    filename = function() {
      paste(input_filename_generic(),'pdf', sep = '.')
    },
  ...
  callModule(downloadCSV, 'downloadFile2', datos$Tabla, input_filename_generic())
  callModule(downloadPAIR, 'downloadPAIR1', datos$aln2, input_filename_pair())
  callModule(downloadFASTA, 'downloadFASTA1', alignedPattern(datos$aln[tabla_rows_selected()]), input_filename_group())
  callModule(downloadFASTA, 'downloadFASTAS', alignedPattern(datos$aln[rows_selected()]), input_filename_results())
}
