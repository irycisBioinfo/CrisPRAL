...
server <- function(input, output) {
  ...
  Primer_Directinput <- eventReactive(input$uploadPrimers,{ c(input$F_Primer1,input$F_Primer2) })
  ...
  PrimerR1_5 <- reactive({
    ...
    return(as.character(PrimerR1_sequences()[1]))
  })
  PrimerR1_3 <- reactive({
    if (is.null(Primer_FileInput())){
      return("Empty")
    }
    return(as.character(PrimerR1_sequences()[2]))
  })
  PrimerR2_5 <- reactive({
    if (is.null(Primer_FileInput())){
      return("Empty")
    }
    return(as.character(PrimerR2_sequences()[2]))
  })
  PrimerR2_3 <- reactive({
    if (is.null(Primer_FileInput())){
      return("Empty")
    } 
    return(as.character(PrimerR2_sequences()[1]))
  })
  ...
  observeEvent(c(input$tabla_rows_selected, input$alnType, input$alnTo), {
    withProgress(message = "Performing analisis", {
      ...
      output$Blast_Search <- renderUI({tags$a(imageOutput("BLASTn.png"), 
