...
server <- function(input, output) {
  ...
  Reference <- reactive({ 
    if(file_upload$upload_Primer_state == 'uploaded'){
      ...
      system(paste(CompletePATH, '/bin/cutadapt/cutadapt -n 2 -g', as.character(PrimerR1_5()), '-a', as.character(PrimerR2_5()),
                   '-o', 'Filtered_Reference.fasta', input$Reference$datapath, sep = ' ' ))
      return('Filtered_Reference.fasta')
    }
    else{
  ...
  observeEvent(c(input$GG), { #---Plotting Graphs
        ...
        Pie_data <- select(datos$Tabla, ID, Freq) %>% filter(Freq > 1) %>% add_row(ID = paste('<1% groups', 
                                                                                              '(', count(Other['Freq']) , ')',
                                                                                              sep = ''), 
                                                                                   Freq = sum(Other['Freq'])) 
        ...
        PC <- plot_ly(Pie_data, labels = ~ID, values = ~Freq, type = 'pie',
                      textposition = NULL,
                      hoverinfo = 'text',
                      marker = list(colors = colors, line = list(color = '#FFFFFF', width = 1)), 
                      text = ~ID ,showlegend = FALSE) %>%
                      layout(title = 'Main Groups',
                             xaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE),
                             yaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE))
        
        output$Pie_summary <- renderPlotly({print(PC)})
        ...
        IL = plot_ly(Insert_per_loci, y = ~Total_Count, x = ~Position, type = 'bar' ) %>%
          layout(yaxis = list(title = 'Count'), xaxis = list(range = c(1, 250)))
