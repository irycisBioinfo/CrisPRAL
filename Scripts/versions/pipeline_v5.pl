...
if ($Adapter_R1 eq 'Empty') {
	...
	system("echo -Adapter filtering");

	system("$PATH/bin/cutadapt/cutadapt -j 0 -g $Adapter_R1 -G $Adapter_R2 -n 2 -o R1_filteredA.fastq -p R2_filteredA.fastq $r1 $r2");
...
if ($F_Primer1 eq 'Empty') {
	...
	system("$PATH/bin/cutadapt/cutadapt -j 0 -m 10 -g $R_Primer1 -a $R_Primer2 -n 2 -o R2_filteredP.fastq goodA_R2.fastq");
